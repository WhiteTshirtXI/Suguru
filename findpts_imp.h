...
{
  //lailai deconstruct, this subroutine is used.
  findpts_local_setup(&fd->local,elx,n,nel,m,bbox_tol,local_hash_size,
                      npt_max, newt_tol);//lailai, it seems that this subroutine has done nothing, only declaration exist, no definition is here, perhaps is just used to change the type of arguments. the declaration is in findpts_local.h
