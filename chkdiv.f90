...
if (myid.eq.int(divmax_all(2))) then
  ...
  111 format('Maximal divergence for main mesh at i = ',I5,' j = ', I5,' k = ',I5)
  222 format('Divergence for main mesh: Tot = ',e13.6,' Max = ',e13.6,' Rank = ',I3)
endif
!
if (divtot_all .gt. 1.e-3) then
  if (myid .eq. divmax_all(2)) then
    write(6,*) 'Fatal error: total divergence for main mesh > 1.e-3!'
