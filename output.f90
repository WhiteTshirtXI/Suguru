...
real    :: visc4(1:jmax,1:kmax)
real    :: forcey4(1:jmax,1:kmax)
real    :: forcez4(1:jmax,1:kmax)
real    :: indic4(1:jmax,1:kmax)

real    :: u4r(0:(jtot+1),0:(kmax+1))
real    :: v4r(0:(jtot+1),0:(kmax+1))
real    :: w4r(0:(jtot+1),0:(kmax+1))
real    :: p4r(0:(jtot+1),0:(kmax+1))
real    :: visc4r(0:(jtot+1),0:(kmax+1))
real    :: forcey4r(0:(jtot+1),0:(kmax+1))
real    :: forcez4r(0:(jtot+1),0:(kmax+1))
real    :: indic4r(0:(jtot+1),0:(kmax+1))
...
call write2dplane(aux,1,itot/2,'yzi_main',istep)
      ...
      do l=1,dims(2)-1
        ...
        if (myid .eq. sub_rank) then
          do k=1,kmax
            do j=1,jmax
              ...
              w4(j,k)=wnew(iloc,j,k)
              p4(j,k)=pnew(iloc,j,k)
              visc4(j,k)=visc(iloc,j,k)
              forcey4(j,k)=forcey(iloc,j,k)
              forcez4(j,k)=forcez(iloc,j,k)
              indic4(j,k)=indic(iloc,j,k)
            enddo
          enddo
          ...
          call MPI_SSEND(visc4,jmax*kmax,MPI_REAL8,rank,11,comm_cart,error)
          call MPI_SSEND(forcey4,jmax*kmax,MPI_REAL8,rank,12,comm_cart,error)
          call MPI_SSEND(forcez4,jmax*kmax,MPI_REAL8,rank,13,comm_cart,error)
          call MPI_SSEND(indic4,jmax*kmax,MPI_REAL8,rank,14,comm_cart,error)
        endif
        if (myid .eq. rank) then
          ...
          call MPI_RECV(p4,jmax*kmax,MPI_REAL8,sub_rank,10, &
                        comm_cart,status,error)
          call MPI_RECV(visc4,jmax*kmax,MPI_REAL8,sub_rank,11, &
                        comm_cart,status,error)
          call MPI_RECV(forcey4,jmax*kmax,MPI_REAL8,sub_rank,12, &
                        comm_cart,status,error)
          call MPI_RECV(forcez4,jmax*kmax,MPI_REAL8,sub_rank,13, &
                        comm_cart,status,error)
          call MPI_RECV(indic4,jmax*kmax,MPI_REAL8,sub_rank,14, &
                        comm_cart,status,error)
          ...
          do k=1,kmax
            do j=1,jmax
              ...
              visc4r(j+l*jmax,k)=visc4(j,k)
              forcey4r(j+l*jmax,k)=forcey4(j,k)
              forcez4r(j+l*jmax,k)=forcez4(j,k)
              indic4r(j+l*jmax,k)=indic4(j,k)
            enddo
          enddo
        endif
      enddo
      if (myid .eq. rank) then
        do k=1,kmax
          do j=1,jmax
            ...
            w4r(j,k)=wnew(iloc,j,k)
            p4r(j,k)=pnew(iloc,j,k)
            visc4r(j,k)=visc(iloc,j,k)
            forcey4r(j,k)=forcey(iloc,j,k)
            forcez4r(j,k)=forcez(iloc,j,k)
            indic4r(j,k)=indic(iloc,j,k)
          enddo
          !periodic b.c.
          u4r(0,k)     =u4r(jtot,k)
        enddo
        ...
        npoints=0       
          ...
          open(43,file=datadir//'yz_mid_main'//filenumber//'.txt')
          write(43,*) 'VARIABLES = "y","z","u","v","w","p","visc","forcey","forcez","indicator"'
...
!'
          do k=1,kmax
            ...
            do jt=-npoints+1,jtot+npoints
              ...
              write(43,'(10E15.7)') ((jt-0.5)/dyi),((k-0.5)/dzi),u4r(j,k), &
                                   0.5*(v4r(j,k)+v4r(jm,k)),               &
                                   0.5*(w4r(j,k)+w4r(j,km)),p4r(j,k),&
                                   visc4r(j,k),forcey4r(j,k),forcez4r(j,k),indic4r(j,k)                                      
