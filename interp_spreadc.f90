...
implicit none
private
public eulr2lagrc,lagr2eulrc,lagr2eulrindch
contains

subroutine eulr2lagrc
implicit none
  ...
  if (ap(p)%mslv .ne. 0) then
    ...
    do nb=0,8
      if((nb.gt.0.and.ap(p)%mslv.lt.0.and.ap(p)%nb(nb).eq.1) .or. & !slave
        ...
        if (coorzfp<nf) then
          coorxfp = (anb(nbrecv,p)%xfp(l)-boundleftmyid)*dxic
          cooryfp = (anb(nbrecv,p)%yfp(l)-boundfrontmyid)*dyic
          coorzfp =  anb(nbrecv,p)%zfp(l)*dzic
          ilow  = nint( coorxfp - 2. )
          ...
          if (jlow .lt. 1) jlow = 1
          if (klow .lt. 1) klow = 1
          if (ihigh .gt. imaxc) ihigh = imaxc
          if (jhigh .gt. jmaxc) jhigh = jmaxc
          !if (khigh .gt. kmaxc) khigh = kmaxc
          do k=klow,khigh
            coorzs   = (1.*k)-coorzfp
            ...
            do j=jlow,jhigh
              ...
              do i=ilow,ihigh
                ...
                anb(nbrecv,p)%ul(l) = anb(nbrecv,p)%ul(l) + unewc(i,j,k)*kernelxs*kernely*kernelz
                anb(nbrecv,p)%vl(l) = anb(nbrecv,p)%vl(l) + vnewc(i,j,k)*kernelx*kernelys*kernelz
                anb(nbrecv,p)%wl(l) = anb(nbrecv,p)%wl(l) + wnewc(i,j,k)*kernelx*kernely*kernelzs
...
!$omp atomic
                forcexc(i,j,k) = forcexc(i,j,k) + anb(nbrecv,p)%fxl(l)*kernelxs*kernely*kernelz*(anb(nbrecv,p)%area(l)/dVeul)
!$omp atomic
                forceyc(i,j,k) = forceyc(i,j,k) + anb(nbrecv,p)%fyl(l)*kernelx*kernelys*kernelz*(anb(nbrecv,p)%area(l)/dVeul)
!$omp atomic
                forcezc(i,j,k) = forcezc(i,j,k) + anb(nbrecv,p)%fzl(l)*kernelx*kernely*kernelzs*(anb(nbrecv,p)%area(l)/dVeul)
...
!
call updthalosc(forcexc,1)
call updthalosc(forceyc,1)
call updthalosc(forcezc,1)
!
! communicate data in y direction (periodic b.c.'s incorporated)
!
call updthalosc(forcexc,2)
call updthalosc(forceyc,2)
call updthalosc(forcezc,2)
    ...
    do nb=0,8
      if((nb.gt.0.and.ap(p)%mslv.lt.0.and.ap(p)%nb(nb).eq.1) .or. & !slave
        ...
        do l=1,nl
          ...
          if (jlow .lt. 1) jlow = 0
          if (klow .lt. 1) klow = 0
          if (ihigh .gt. imaxc) ihigh = imaxc+1
          if (jhigh .gt. jmaxc) jhigh = jmaxc+1
          if (khigh .gt. kmaxc) khigh = kmaxc+1
          do k=klow,khigh
            coorzs   = (1.*k)-coorzfp
            coorz    = coorzs-0.5           !vert.    distance in grid points
            kernelzs = kernel(coorzs)
...
!$omp atomic
                gxc(i,j,k) = gxc(i,j,k) + anb(nbrecv,p)%xn(l)*kernelx*kernely*kernelz*(anb(nbrecv,p)%area(l)/dVeul)
!$omp atomic
                gyc(i,j,k) = gyc(i,j,k) + anb(nbrecv,p)%yn(l)*kernelx*kernely*kernelz*(anb(nbrecv,p)%area(l)/dVeul)
!$omp atomic
                gzc(i,j,k) = gzc(i,j,k) + anb(nbrecv,p)%zn(l)*kernelx*kernely*kernelz*(anb(nbrecv,p)%area(l)/dVeul)
!!$omp atomic
...
!
return
end subroutine lagr2eulrindch
!
end module mod_interp_spreadc
